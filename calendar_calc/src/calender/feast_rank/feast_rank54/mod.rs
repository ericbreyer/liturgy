...
impl FeastRankResolver for FeastRank54 {
    ...
    fn is_high_festial(&self) -> bool {
        matches!(
            ...
            FeastRank54Inner::Feast {
                ...
                rank: OctaveType::Privileged1,
                ..
            } | FeastRank54Inner::Octave {
                rank: OctaveType::Privileged2,
                ..
            } | FeastRank54Inner::Octave {
                rank: OctaveType::Privileged3,
                ..
            }
        )
    }
    ...
    where
        Self: Sized,
    {
        Some(FeastRank54(FeastRank54Inner::Feast {
                ...
                match *rank2 {
                    ...
                    OctaveType::Privileged2 => match (rank1, is_octave_day) {
                        ...
                        (FeastClass::Double, _) => {
                            ...
                            // non-transitive cycle observed in three-way comparisons for those ranks.
                            return Ok(OccurrenceResult::SecondCommemorationOfFirst);
                        }
                    ...
                    OctaveType::Privileged3 => match rank1 {
                        ...
                        // Semidouble (of Our Lord) -> Major Double (Octave Day) -> Privileged3 Octave Day -> Semidouble (of Our Lord).
                        FeastClass::Semidouble
            ...
            } = other
            {
                match (rank1, rank2) {
                    (FeastClass::FirstClassDouble, FeastClass::FirstClassDouble)
                        if flags1.contains(FeastFlags::OF_OUR_LORD)
                            && !_flags2.contains(FeastFlags::OF_OUR_LORD) =>
                    {
                        return Ok(OccurrenceResult::FirstNothingOfSecond)
                    }
                    (FeastClass::FirstClassDouble, FeastClass::SecondClassDouble) => {
        ...
        match self {
            ...
            FeastRank54Inner::Feast { rank, flags } => {
                ...
                if v && flags.contains(FeastFlags::OF_OUR_LORD) {
                    format!("{} (of Our Lord)", base_name).into()
                } else {
                    base_name.into()
                }
            }
        ...
        match day_type {
            DayType::Feria => {
                ...
                let rank = match rank {
                    ...
                    "ordinary" | "III" => FeriaClass::Lesser, // Ordinary ferias
                    "IV" => FeriaClass::Lesser, // Treat IV as Lesser for compatibility
...
mod tests {
    ...
    fn test_octave_day_vs_first_class_sunday() {
        ...
        let sunday =
            FeastRank54::new_with_context("I", &DayType::Sunday, &LiturgicalContext::new());
    ...
    fn test_first_class_double_vs_second_class_sunday_vs_third_class_octave() {
        ...
        assert_eq!(result.commemorations, vec!["Sacred Heart".to_string()]);
    }
